...
async function api(path, options = {}) {
  ...
  catch { throw new Error(response.ok ? "The service returned an unexpected response." : `The service is temporarily unavailable (${response.status}). Please retry.`); }
...
async function beginOnboarding(path) {
  try {
    ...
    await refresh(path === "discover_for_me" ? "Reference venture preparation started in the background." : "Upload completed. CrewAI is analyzing the product in the background; you can safely leave this page.");
  } catch (error) {
    say(error.message, true);
  }
}
...
function renderDashboard() {
  ...
  const latestVisual = (state.runs || []).flatMap((run) => run.receipt?.outputArtifacts || []).find((artifact) => artifact.contentType?.startsWith("image/"));
  const visualPublished = latestVisual && state.venture.publicVisualUri?.endsWith(latestVisual.storedUri.split("/").pop());
  const adPackages = state.advertising?.packages || [];
  const latestAdPackage = adPackages[0];
  const onboardingBusy = ["queued", "running"].includes(state.onboarding?.runtimeStatus);
  app.innerHTML = `<div class="autopilot-banner"><div><strong>Venture Runtime · ${runtime.mode}</strong><p>${onboardingBusy ? "CrewAI is analyzing the uploaded product in the background." : `${liveAgents} running now · ${completedAgents} completed real agent runs · reasoning: ${esc(state.connections.agents.mode)} · execution: n8n.`}</p></div><span class="autopilot-state">● ${onboardingBusy || liveAgents ? "CREW ACTIVE" : durable ? "n8n EXECUTION READY" : "MONITORING"}</span></div>${state.onboarding?.runtimeStatus === "needs_attention" ? `<div class="approval-box"><strong>Product analysis needs attention</strong><p class="muted">${esc(state.onboarding.runtimeError?.message || "The background run failed.")}</p></div>` : ""}<div class="card hero"><p class="eyebrow">Your operating venture</p><h2>${esc(state.venture.name)}</h2><p>${esc(state.venture.promise)}</p><div class="work-summary"><div><strong>Offer</strong><small>${onboardingBusy ? "crew preparing" : completedAgents ? "crew-produced" : "reference draft"}</small></div><div><strong>Funnel</strong><small>${funnelLive ? `live · v${runtime.funnel.version}` : onboardingBusy ? "being prepared" : "not deployed"}</small></div><div><strong>Fulfillment</strong><small>${durable ? "n8n + protected" : executionConfigured ? "configured · unverified" : "not connected"}</small></div><div><strong>Optimization</strong><small>${durable ? "CrewAI proposal + approval" : executionConfigured ? "ready to retry" : "not connected"}</small></div></div>${runtime.funnel ? `<div class="action-row"><a class="button-link" href="/v/${encodeURIComponent(runtime.funnel.slug)}" target="_blank">Open live funnel</a><button class="secondary" id="optimize" ${state.connections.agents.status !== "ready" || !executionConfigured ? "disabled" : ""}>Run CrewAI optimizer</button></div>` : ""}</div>
  ...
  ${runtime.leads.length ? `<div class="card" style="margin:18px 0"><div class="section-head compact"><div><p class="eyebrow">Owner CRM</p><h2>Recent consented leads</h2></div><span class="chip">${runtime.leads.length} total</span></div>${runtime.leads.slice(0,10).map((lead) => `<div class="connection"><div><strong>${esc(lead.email)}</strong><small>${esc(lead.source)} · ${when(lead.createdAt)} · consent ${lead.consent ? "recorded" : "missing"}</small></div>${statusChip(lead.consent ? "ready" : "needs_attention")}</div>`).join("")}</div>` : ""}
  ${latestVisual ? `<div class="card visual-review" style="margin:18px 0"><div><p class="eyebrow">Generated visual</p><h2>${visualPublished ? "Live funnel visual" : "Private owner preview"}</h2><p class="muted">${visualPublished ? "This exact artifact is visible on the public funnel." : "The image is visible only to the owner until you explicitly publish it."}</p><div class="action-row">${runtime.funnel ? `<a class="button-link secondary" href="/api/funnel/${encodeURIComponent(runtime.funnel.slug)}/preview?visual=${encodeURIComponent(latestVisual.storedUri.split("/").pop())}" target="_blank">Preview funnel with image</a>` : ""}${visualPublished ? `<a class="button-link" href="${esc(state.venture.publicVisualUri)}" target="_blank">Open public image</a>` : `<button id="publish-visual" data-private-uri="${esc(latestVisual.storedUri)}">Approve & publish on funnel</button>`}<a class="button-link secondary" href="#runs">View evidence receipt</a></div></div><img class="asset" src="${esc(latestVisual.storedUri)}" alt="Generated private visual for ${esc(state.venture.name)}"></div>` : ""}
  ${state.onboarding?.product ? `<div class="card" style="margin:18px 0"><p class="eyebrow">Customer delivery map</p><h2>Two PDFs, two different moments</h2><div class="grid two"><div class="artifact"><small>Email signup · free</small><h3>${esc(runtime.funnel?.content?.leadMagnet || "Generated diagnostic")}</h3><p class="muted">A generated lead magnet starts the relationship. It is not the uploaded paid product.</p></div><div class="artifact"><small>Successful Stripe payment · protected</small><h3>${esc(state.onboarding.product.originalName)}</h3><p class="muted">The exact uploaded file is delivered through an expiring, download-limited link.</p></div></div></div>` : ""}
  ${latestAdPackage ? adPackageCard(latestAdPackage) : `<div class="card" style="margin:18px 0"><p class="eyebrow">Supervised acquisition</p><h2>Prepare a Meta campaign package</h2><p class="muted">This creates three reviewable ad variants from the current funnel and approved image. It does not contact Meta or spend money.</p><form id="prepare-ad-package" class="grid three"><label>Daily planning limit (€)<input name="dailyBudget" type="number" min="1" max="1000" step="1" value="10"></label><label>Duration (days)<input name="durationDays" type="number" min="1" max="30" step="1" value="7"></label><label>Target countries<input name="countries" value="CZ" placeholder="CZ, DE"></label><div class="action-row" style="grid-column:1/-1"><button ${!runtime.funnel || !latestVisual ? "disabled" : ""}>Generate ad previews</button><span class="muted">${!runtime.funnel || !latestVisual ? "A live funnel and generated visual are required." : "No external action."}</span></div></form></div>`}
  ...
  document.querySelector("#publish-visual")?.addEventListener("click", async (event) => { event.target.disabled = true; event.target.textContent = "Publishing exact artifact…"; try { const filename = event.target.dataset.privateUri.split("/").pop(); await api(`/api/assets/${encodeURIComponent(filename)}/publish`, { method:"POST", body:"{}" }); await refresh("The approved visual is now visible on the public funnel."); } catch(error) { await refresh(); say(error.message, true); } });
  document.querySelector("#prepare-ad-package")?.addEventListener("submit", async (event) => { event.preventDefault(); const form = new FormData(event.target); const input = { dailyBudget:Number(form.get("dailyBudget")), durationDays:Number(form.get("durationDays")), countries:form.get("countries") }; try { await api("/api/ads/packages", { method:"POST", body:JSON.stringify(input) }); await refresh("Ad package prepared. Review the exact image, copy, targeting and budget before approving a paused Meta draft."); } catch(error) { say(error.message, true); } });
  document.querySelector("#approve-meta-draft")?.addEventListener("click", async (event) => { event.target.disabled = true; event.target.textContent = "Creating PAUSED Meta draft…"; try { await api(`/api/ads/packages/${event.target.dataset.packageId}/approve-draft`, { method:"POST", body:JSON.stringify({version:event.target.dataset.version}) }); await refresh("Meta campaign, ad set and ads were created in PAUSED state. No spend has started."); } catch(error) { await refresh(); say(error.message, true); } });
  document.querySelector("#replace-ad-package")?.addEventListener("click", async (event) => { event.target.disabled = true; try { const current = latestAdPackage; await api("/api/ads/packages", { method:"POST", body:JSON.stringify({dailyBudget:current.budget.daily,durationDays:current.budget.durationDays,lifetimeBudget:current.budget.lifetime,countries:current.targeting.countries.join(",")}) }); await refresh("A new immutable ad package was prepared from the latest generated visual."); } catch(error) { await refresh(); say(error.message, true); } });
  ...
  if (onboardingBusy) window.setTimeout(() => refresh().catch((error) => say(error.message, true)), 4000);
}
...
function renderVenture() {
  ...
  const ventureForm = document.querySelector("#venture-form");
  ventureForm.querySelector('[name="sourceRights"]').closest("label").insertAdjacentHTML("afterend", `<fieldset class="offer-editor"><legend>EU/EEA ad transparency</legend><p class="muted">Public legal names Meta displays for ads targeting Europe. These are business facts, not API credentials.</p><label>Person or organisation being promoted<input name="dsa-beneficiary" value="${esc(v.adTransparency?.beneficiary || "")}" placeholder="Truthful legal name"></label><label>Person or organisation paying for the ads<input name="dsa-payor" value="${esc(v.adTransparency?.payor || "")}" placeholder="Truthful legal name"></label></fieldset>`);
  ventureForm.onsubmit = async (event) => {
    event.preventDefault(); const form = new FormData(event.target);
    const input = { name:form.get("name"), audience:form.get("audience"), promise:form.get("promise"), sourceRights:form.get("sourceRights"), adTransparency:{beneficiary:form.get("dsa-beneficiary"),payor:form.get("dsa-payor")}, offers:v.offers.map((offer,index)=>({name:form.get(`offer-name-${index}`),price:Number(form.get(`offer-price-${index}`)),paymentUrl:form.get(`offer-url-${index}`)})) };
    try { await api("/api/venture", { method:"POST", body:JSON.stringify(input) }); await refresh("Business brief, checkout and ad transparency saved."); } catch (error) { say(error.message, true); }
  };
}
