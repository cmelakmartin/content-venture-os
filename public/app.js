...
function renderVenture() {
  ...
  app.innerHTML = `<div class="grid two"><div class="card"><h2>Operating brief</h2><p class="muted">This is the reusable customer input layer. A future onboarding wizard would collect the same decisions.</p><form id="venture-form"><label>Venture name<input name="name" value="${esc(v.name)}"></label><label>Audience<textarea name="audience">${esc(v.audience)}</textarea></label><label>Promise<textarea name="promise">${esc(v.promise)}</textarea></label><label>Source rights and limits<textarea name="sourceRights">${esc(v.sourceRights)}</textarea></label><button>Save operating brief</button></form></div><div><div class="card"><h2>Offer ladder</h2>${v.offers.map(o=>`<div class="offer"><div><strong>${esc(o.name)}</strong><small>${esc(o.role)}</small></div><span class="price">${o.price ? money(o.price) : "Free"}</span></div>`).join("")}</div><div class="card" style="margin-top:18px"><h3>Permanent launch rule</h3><p class="muted">Validate demand organically before buying a domain or activating ads. Visuals, domain registration, public publishing and ad spend remain separate approvals.</p></div></div></div>`;
  document.querySelector("#venture-form").onsubmit = async (event) => { event.preventDefault(); const input = Object.fromEntries(new FormData(event.target)); try { await api("/api/venture", { method:"POST", body:JSON.stringify(input) }); await refresh("Operating brief saved."); } catch (error) { say(error.message, true); } };
}
...
function statusChip(status){ const good=["completed","ready","test_links_ready","approved","applied","webhook_ready"].includes(status); const stop=["stopped","failed","rejected","unavailable"].includes(status); return `<span class="status ${good?'good':stop?'stop':'wait'}">${esc(statusLabel(status))}</span>`; }
